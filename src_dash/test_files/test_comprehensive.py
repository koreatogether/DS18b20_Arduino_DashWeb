...
def test_serial_communication(port_name, duration=10):
    ...
    try:
        ...
        while time.time() - start_time < duration:
            ...
            # 2초마다 명령 전송
            if (current_time - last_command_time > 2 and 
                command_index < len(test_commands)):
...
def main():
    ...
    if result['success']:
        ...
        if result['sensor_data'] > 0:
            print(f"  🌡️ 센서 데이터 정상 수신 ({result['sensor_data']}개)")
        else:
            print("  ⚠️ 센서 데이터 미확인 - 센서 연결 상태 점검 필요")
            
        if result['command_responses'] > 0:
            print(f"  📤 명령 응답 정상 ({result['command_responses']}개)")
        else:
