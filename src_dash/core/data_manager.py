...
def create_snapshot_function(
    arduino: Any, arduino_connected_ref: Dict[str, bool]
) -> Callable[
    [],
    Tuple[
        str,
    ...
    def snapshot() -> Tuple[
        ...
        if arduino_connected and arduino.is_healthy():
            ...
            connection_status = (
                f"🟢 Arduino 연결됨 (데이터: {stats['sensor_data_count']}개)"
            )
            connection_style = {
                "textAlign": "center",
                "margin": "10px",
            ...
            print(
                f"🔍 실제 데이터 사용: 현재온도={len(current_temps)}개, 최신데이터={len(latest_data)}개"
            )
        else:
