...
class ArduinoSerial:
    ...
    def disconnect(self):
        ...
        if self.serial_connection:
            try:
                self.serial_connection.close()
            except Exception:  # noqa: E722 - 광범위 예외 캡처 (연결 해제 시도)
                pass
            self.serial_connection = None
        ...
        try:
            ...
            with self.data_lock:
                if msg_type == "sensor":
                    ...
                    self.logger.info(
                        f"✅ JSON 센서 저장: ID={record['sensor_id']}, 온도={record['temperature']}°C"
                    )
        ...
        with self.data_lock:
            if msg_type == "SENSOR_DATA" and len(parts) >= 4:
                try:
                    ...
                    self.sensor_data.append(record)
                    self.logger.info(
                        f"✅ CSV 센서 저장: ID={record['sensor_id']}, 온도={record['temperature']}°C"
                    )
