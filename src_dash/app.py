...
@app.callback(
    [Output('main-content', 'children'), Output('ui-version-store', 'data'), Output('mode-indicator', 'children'), Output('mode-feedback', 'children')],
    [Input('btn-ver-1', 'n_clicks'), Input('btn-ver-2', 'n_clicks')],
...
def update_main_layout(n1, n2, current_version):
    print(f"\n{'='*50}")
    print(f"🔍 [LAYOUT_CALLBACK] 레이아웃 전환 콜백 호출됨")
    ...
    if not ctx.triggered:
        button_id = 'initial'
        print(f"🔍 [LAYOUT_CALLBACK] 초기 로드 - 기본값 사용")
    else:
        trigger_info = ctx.triggered[0]
        button_id = trigger_info['prop_id'].split('.')[0]
        trigger_value = trigger_info['value']
    ...
    try:
        ...
        if button_id == 'btn-ver-2' and n2 and n2 > 0:
            print(f"🌙🌙🌙 [NIGHT_MODE] Night mode 버튼 클릭 감지!")
            print(f"🌙 [NIGHT_MODE] 클릭 수: {n2}")
            print(f"🌙 [NIGHT_MODE] v2 레이아웃 전환 시작...")
            
            # create_layout_v2 함수 존재 확인
            try:
                print(f"🔍 [NIGHT_MODE] night_sections.night_layout에서 create_layout_v2 import 시도...")
                from night_sections.night_layout import create_layout_v2
                print(f"✅ [NIGHT_MODE] create_layout_v2 함수 import 성공!")
            except ImportError as ie1:
                print(f"⚠️ [NIGHT_MODE] 첫 번째 import 실패: {ie1}")
                try:
                    print(f"🔍 [NIGHT_MODE] night_layout에서 create_layout_v2 import 시도...")
                    from night_layout import create_layout_v2
                    print(f"✅ [NIGHT_MODE] create_layout_v2 함수 import 성공 (대체 경로)!")
                except ImportError as ie2:
                    print(f"❌ [NIGHT_MODE] 두 번째 import도 실패: {ie2}")
                    print(f"❌ [NIGHT_MODE] Night 모드를 사용할 수 없습니다. Day 모드로 유지합니다.")
                    # Night 모드 실패 시 Day 모드로 fallback
            ...
            print(f"🔍 [NIGHT_MODE] 포트 옵션 가져오는 중...")
            try:
                ...
                current_port_options, current_default = INITIAL_PORT_OPTIONS, selected_port
            
            # v2 레이아웃 생성
            print(f"🔍 [NIGHT_MODE] v2 레이아웃 생성 중...")
            try:
                layout_v2 = create_layout_v2(
                    current_port_options, selected_port or current_default, current_default, 
                    app, arduino, ARDUINO_CONNECTED, COLOR_SEQ, TH_DEFAULT, TL_DEFAULT, _snapshot
                )
                print(f"✅✅✅ [NIGHT_MODE] v2 레이아웃 생성 완료!")
                print(f"🌙 [NIGHT_MODE] Night 모드로 전환됩니다!")
                print(f"{'='*50}\n")
                return layout_v2, UIMode.NIGHT.value, "🌙 Night 모드 활성화", f"🌙 Night 버튼이 클릭되었습니다! (클릭 수: {n2})"
                ...
                layout_v1 = create_layout_v1(INITIAL_PORT_OPTIONS, selected_port, INITIAL_PORT_VALUE)
                return layout_v1, UIMode.DAY.value, "☀️ Day 모드 (오류로 인한 복원)", "❌ Night 모드 오류 발생"
            
        # Day 모드 버튼 클릭 또는 기본값 처리
        else:
            if button_id == 'btn-ver-1' and n1 and n1 > 0:
                print(f"☀️ [DAY_MODE] Day mode 버튼 클릭 감지!")
                print(f"☀️ [DAY_MODE] 클릭 수: {n1}")
            else:
                print(f"☀️ [DAY_MODE] 기본 v1 레이아웃 로드")
            
            print(f"☀️ [DAY_MODE] v1 레이아웃 생성 중...")
            layout_v1 = create_layout_v1(INITIAL_PORT_OPTIONS, selected_port, INITIAL_PORT_VALUE)
            print(f"✅ [DAY_MODE] v1 레이아웃 생성 완료!")
            print(f"{'='*50}\n")
            if button_id == 'btn-ver-1' and n1 and n1 > 0:
                return layout_v1, UIMode.DAY.value, "☀️ Day 모드 활성화", f"☀️ Day 버튼이 클릭되었습니다! (클릭 수: {n1})"
            else:
                return layout_v1, UIMode.DAY.value, "☀️ Day 모드 (기본)", "애플리케이션 시작"
        ...
        print(f"{'='*50}\n")
        # 오류 발생 시 기본 레이아웃 반환
    return create_layout_v1(INITIAL_PORT_OPTIONS, selected_port, INITIAL_PORT_VALUE), UIMode.DAY.value, "❌ 오류 발생 (Day 모드로 복원)", "❌ 시스템 오류 발생"
