...
def create_sensor_cards_with_buttons(TH_DEFAULT=55.0, TL_DEFAULT=-25.0):
    ...
    for i in range(1, 9):
        ...
        card = html.Div([
            ...
            html.Div([
                html.Div([
                    dcc.Graph(id=f'sensor-{i}-mini-graph', figure=fig, style={'height': '200px'}, config={'displayModeBar': False})
                ], style={'flex': '1'}),
                html.Div([
                    html.Div(f"TH {TH_DEFAULT:.1f}",
                            style={'color': 'red', 'fontSize': '16px', 'marginBottom': '15px', 'marginTop': '-40px'}),
                    html.Div(id=f'sensor-{i}-current-temp', children="--°C",
                        style={'color': 'white', 'fontSize': '16px', 'fontWeight': 'bold', 'marginBottom': '10px'}),
                    html.Div("0", style={'color': '#aaa', 'fontSize': '16px', 'marginBottom': '8px', 'textAlign': 'center'}),
