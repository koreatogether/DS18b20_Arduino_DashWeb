...
bool JsonCommunication::parseJsonCommand(const char *jsonStr, char *type, char *content, int maxTypeSize, int maxContentSize)
{
  ...
  if (strlen(typeValue) >= (size_t)maxTypeSize)
  {
    return false;
  }

  // Safe copy with bounds check (replaces strcpy)
  if (maxTypeSize > 0) {
    size_t copyLen = min(strlen(typeValue), (size_t)(maxTypeSize - 1));
    memcpy(type, typeValue, copyLen);
    type[copyLen] = '\0';
  }

  // 전체 내용을 content로 복사
  if (strlen(jsonStr) >= (size_t)maxContentSize)
  {
    return false;
  }
  ...
  // Safe copy with bounds check (replaces strcpy)
  if (maxContentSize > 0) {
    size_t contentLen = min(strlen(jsonStr), (size_t)(maxContentSize - 1));
    memcpy(content, jsonStr, contentLen);
